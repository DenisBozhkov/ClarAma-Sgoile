...
static LPTSTR NewString(LPTSTR str)
{
    ...
    return _fmemmove(tmp, str, _tcslen(str));
}
...
static LPTSTR NewStringArray(LPTSTR szzStr)
{
    ...
    return _fmemmove(tmp, szzStr, iLen);
}
...
static VOID AdjustParentColWidth(HWND hwnd, INT col, INT row, LPTSTR lpszValue)
{
    ...
    {
        int count = 1;
        TCHAR tbuffer[_tcslen(lpszValue) + 1];
...
SIMPLEGRIDAPI ATOM WINAPI InitSimpleGrid(HINSTANCE hInstance)
{
    WNDCLASSEX wcex;
...
SIMPLEGRIDAPI HWND WINAPI New_SimpleGrid(HWND hParent, DWORD dwID)
{
...
static LRESULT Grid_OnSetItemData(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (dwColType)
    {
        ...
        case GCT_CHECK:
            ...
            break;
        case GCT_IMAGE:
            _sntprintf(buf, NELEMS(buf), _T("%d"), (int)lpSGitem->lpCurValue);//DWM 1.9: was _stprintf
            lpszValue = buf;
...
static LRESULT Grid_OnGetItemData(HWND hwnd, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (GetColType(col))
    {
        ...
        case GCT_LINK:
        {
            ...
            _fmemcpy((LPTSTR)lpSGitem->lpCurValue, lpgi->lpszCurValue, iLen + 1); // copy the terminating null also
            return iLen;
        }
        case GCT_CHECK:
